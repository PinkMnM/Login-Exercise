...
if app.config["SQLALCHEMY_DATABASE_URI"] is None:
    app.config["SQLALCHEMY_DATABASE_URI"] = get_database_uri(
        "feedback", cred_file=None, save=False
    )
...
def new_feedback(username: str):
    ...
    if form.validate_on_submit():
        ...
        feedback = Feedback(
            **data, from_username=session["username"], to_username=username
        )
    ...
    return render_template(
        "feedback.html", form=form, username=session["username"], fusername=username
    )
...
def update_feedback(feedback_id: int):
    ...
    return render_template(
        "feedback.html", form=form, feedback=feedback, username=session["username"]
    )
...
if __name__ == "__main__":
    if len(sys.argv) >= 3:
        app.config["SQLALCHEMY_DATABASE_URI"] = get_database_uri(
            sys.argv[3] if len(sys.argv) > 3 else "feedback", sys.argv[1], sys.argv[2]
        )
